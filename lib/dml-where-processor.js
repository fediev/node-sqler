/**
 * @file dml builder where operator processors
 */

const { escape } = require('./util');
const { makeConditionsString } = require('./dml-helper');
...
// processorFactory : (primitiveProcessor) => processor
...
// `squey` should be the result of `subquery()` and include parentheses.
...
/**
 * Generate `OR` expresion. Used in `sqler.sqlWhere()`.
 * Depending on the number of params, `or()` generates the differnt result.
 *   `or(A)` --> 'OR A'
 *   `or(A, B)` --> '(A OR B)'
