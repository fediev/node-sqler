...
function sqlTable(tb) {
  ...
  if (typeof tb === 'object') {
    const tbName = Object.keys(tb)[0];
    const tbAlias = tb[tbName].trim();
    return `${tbName} AS ${tbAlias}`;
  }
...
function parseJoin(tb, join) {
  ...
  const firstTable =
    typeof tb === 'object' ? Object.values(tb)[0].trim() : tb.trim();
  const secondTable =
    typeof join.tb === 'object'
      ? Object.values(join.tb)[0].trim()
      : join.tb.trim();
...
function sqlSelectFields(fields) {
  ...
  // TODO: check alias needs backtick
